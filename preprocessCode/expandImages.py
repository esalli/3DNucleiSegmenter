...
for sph in c.SPHEROIDS:
    ...
    sitk.WriteImage(GTExpanded,c.PREPROCESSED_GT_DATADIR+sph+'_GT_expanded_nearest_3.nrrd',useCompression=True)
    ...
    # use nearest neighbor interpolation for the slices near the bottom and top of the image stack because itk.morphological_contour_interpolator does not seem to handle them properly
    #GTExpandedSparse[:,:,0]=GTExpanded[:,:,0]
    #GTExpandedSparse[:,:,1]=GTExpanded[:,:,1]
    #GTExpandedSparse[:,:,2]=GTExpanded[:,:,2]
    #GTExpandedSparse[:,:,3]=GTExpanded[:,:,3]
    #GTExpandedSparse[:,:,4]=GTExpanded[:,:,4]
    
    #GTExpandedSparse[:,:,GTExpanded.GetSize()[2]-1]=GTExpanded[:,:,GTExpanded.GetSize()[2]-1]
    #GTExpandedSparse[:,:,GTExpanded.GetSize()[2]-2]=GTExpanded[:,:,GTExpanded.GetSize()[2]-2]
    #GTExpandedSparse[:,:,GTExpanded.GetSize()[2]-3]=GTExpanded[:,:,GTExpanded.GetSize()[2]-3]
    #GTExpandedSparse[:,:,GTExpanded.GetSize()[2]-4]=GTExpanded[:,:,GTExpanded.GetSize()[2]-4]
    #GTExpandedSparse[:,:,GTExpanded.GetSize()[2]-5]=GTExpanded[:,:,GTExpanded.GetSize()[2]-5]
    
    
    sitk.WriteImage(GTExpandedSparse,c.PREPROCESSED_GT_DATADIR+sph+'_GT_expanded_sparse_3.nrrd',useCompression=True)
...
#    itk.imwrite(filled,c.PREPROCESSED_GT_DATADIR+sph+'_GT_expanded.nrrd' , compression=True)
    ...
    #in order to change value we require that 1) differenceImage > 0 
    #maskImage_np=differenceImage_np*(GTExpanded_np>0)
    ...
    #spheroids
    spheroid=sitk.ReadImage(c.PREPROCESSED_SPHEROIDS_DATADIR+sph+'_rs_spheroid.nrrd')
    spheroidExpanded=sitk.Expand(spheroid,expandFactors=(1,1,h))
    sitk.WriteImage(spheroidExpanded,c.PREPROCESSED_SPHEROIDS_DATADIR+sph+'_rs_spheroid_expanded_3.nrrd',useCompression=True)
